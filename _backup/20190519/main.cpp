...
int mode(6); //0なら歩行するだけ、1なら全体の流れ、2ならタイマー制御、3なら立つだけ、4と5はカスタム
...
/************************固定パラメーター*******************/
const double ROPE_STEP_HEIGHT(0.40);
...
#endif
{
  ...
  oled.clear();
  if (field == 0)
    oled.printf("BLUE FIELD");
  else
    oled.printf("RED FIELD");
  ...
  //                                   MODE 1                                           //
  /**************************************************************************************/
  else if (mode == ALL_ROTATE) //全体の流れ(カクカク行くver)
  {
    /**************************リスタート用*********************/
    if (state != 0)
      Body_Up_LegType0(X0, Y0, Y1, ALL_HEIGHT);
  ...
  //                                   MODE 6                                           //
  /**************************************************************************************/
  else if (mode == ALL_SMOOTH) //全体の流れ(なめらかにver)
  {
    /**************************リスタート用*********************/
    if (state != 0)
      Body_Up_LegType0(X0, Y0, Y1, ALL_HEIGHT);
