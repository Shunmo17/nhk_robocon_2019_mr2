...
#endif
	bno.get_angles();
	std::cout << (int)bno.euler.roll << " : " << (int)bno.euler.pitch << " : " << (int)bno.euler.yaw << std::endl;
					   ...
					   int step_times, double wait_time)
{
	...
	for (int i(1); i <= step_times + 1; i++)
	{
		...
		// 回転座標を設定
		double angleKp[3] = {0.1, 0.1, 0.05}; // 姿勢補正のP制御のゲイン,(ロール角,ピッチ角,ヨー角)
		...
		if(euler3param2-euler3param[2]>180)euler3param[2] = euler3param2 - 360;
		else if(euler3param2-euler3param[2]<-180)euler3param[2] = euler3param2+360;
		else euler3param[2]=euler3param2;
		...
		rotationmatrix(rotmatrix3, -eulersum[0]+10, eulersum[1], -eulersum[2]);
		//}
