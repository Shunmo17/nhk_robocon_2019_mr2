...
#endif
{
  ...
  else if (mode == ALL_2) //全体の流れ(なめらかにver)
  {
    ...
    /***********************************************************/
    Body_Up_LegType0(X0, Y0, Y1, 0.18);
    wait(0.5);
    //Wait_gerge_setting(); //ゲルゲセッティング
    Receive_gerge(); //ゲルゲ受け渡し
    //Wait_UB();
  ...
  ALL_2_STATE_2:
    ...
    LegType0_to_Dainozi();
    if (field == 0)
      Rotation_Deg(X0, Y0, 0.30, -14.0, 6); //左回り90度回転
    else
      Rotation_Deg(X0, Y0, 0.30, 14.0, 6); //右回り90度回転
  ...
  ALL_2_STATE_3:
    ...
    do
    {
      if (y_now < 6.6 * 1.1 - STEP_LENGTH * 2 * 0.5) //2歩分以上手前
      {
    ...
    } while (y_now < 6.6 * 1.1 - STEP_LENGTH * 0.5); //1歩分以上手前
    Output_Coordinate(X0, -Y1, rf.adjust,
                      X0, -Y1, rb.adjust,
                      X0, -Y1 + STEP_LENGTH / 2, lb.adjust,
                      X0, -Y1 + STEP_LENGTH / 2, lf.adjust,
  ...
  { //赤外線センサの値表示
    while (1)
    {
      ...
      oled.printf("hand:%2.2lfcm", Hand_distance());
      wait(0.1);
    }
  }
  /**************************************************************************************/
  ...
  { //gerge掲げ
    wait(2);
    while (button_white.read() == 0)
    {
      oled.clear();
      oled.printf("Push Button");
      wait(0.1);
    }
    Raise_gerge();
  }
