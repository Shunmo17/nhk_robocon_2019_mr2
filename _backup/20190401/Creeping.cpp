...
//walk_times：繰り返し回数。この回数をnとすると、2n + step_lengthだけ進むはず(実際には全然進まない)
{
    ...
    while (stage < 4)
    {
        if (mode == 1)
        {
            if (y_now > 2.5 && stage == 0)
                stage++; //stage=1に
            else if (y_now > 4.5 && stage == 1)
                stage++; //stage=2に
            else if (x_now > 2.225 && stage == 2)
                stage++; //stage=3に
            else if (y_now > 7.0 && stage == 3)
                stage++; //stage=4に
        ...
        /***********************************************************************/
        if (mode == 5 && slope_flag == 1 && abs(rb.adjust) <= 0.02) //坂を登り終えて1サイクル経ったらbreak
        {
            break;
