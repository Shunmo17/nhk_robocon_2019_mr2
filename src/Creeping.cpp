...
double extend_leg_x(0.0);
double extend_leg_y(0.0);
...
//step_height：足を上げたときの高さ
{
    ...
    {
        /***********************ALL_2用***********************/
        //砂丘のはじめの2歩は重心前寄り、その後重心後ろより、最後に重心をもとに戻す
        if (mode == ALL_2 && state == 2 && i <= 2)
            adjust_y = 0.1;
        else if (mode == ALL_2 && state == 2 && i >= 3 && i <= 4)
            adjust_y = 0.01;
        else if (mode == ALL_2 && state == 2 && i >= 5)
            adjust_y = ADJUST_Y;
        ...
        if (mode == ALL_2 && state == 2 && i >= 4)
        {
            ...
            //step_length = 0.32; //歩幅を小さくしてロープの前に調整→転ぶからコメントアウトした(5/23)
        }
        /*****************************************************/
                           ...
                           double creeping_step_times, double creeping_wait_time)
{
    ...
    Output_Coordinate(x0 + extend_leg_x, -y1 - extend_leg_y - adjust_y, rf.adjust + step_height,
                      x0, -y1 + adjust_y, rb.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_left, lb.adjust,
                      x0, -y1 - adjust_y + step_length / 2 * ratio_left, lf.adjust,
                      all_height,
    ...
    Output_Coordinate(x0 + extend_leg_x, -y1 + extend_leg_y - adjust_y + step_length * ratio_right, rf.adjust + step_height,
                      x0, -y1 + adjust_y, rb.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_left, lb.adjust,
                      x0, -y1 - adjust_y + step_length / 2 * ratio_left, lf.adjust,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y + step_length / 2 * ratio_right, rf.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_right, rb.adjust,
                      x0 + extend_leg_x, -y1 + extend_leg_y + adjust_y + step_length * ratio_left, lb.adjust + step_height,
                      x0, -y1 - adjust_y, lf.adjust,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y + step_length / 2 * ratio_right, rf.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_right, rb.adjust,
                      x0 + extend_leg_x, -y1 - extend_leg_y + adjust_y, lb.adjust + step_height,
                      x0, -y1 - adjust_y, lf.adjust,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y + step_length / 2 * ratio_right, rf.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_right, rb.adjust,
                      x0, -y1 + adjust_y, lb.adjust,
                      x0 + extend_leg_x, -y1 - extend_leg_y - adjust_y, lf.adjust + step_height,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y + step_length / 2 * ratio_right, rf.adjust,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_right, rb.adjust,
                      x0, -y1 + adjust_y, lb.adjust,
                      x0 + extend_leg_x, -y1 + extend_leg_y - adjust_y + step_length * ratio_left, lf.adjust + step_height,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y, rf.adjust,
                      x0 + extend_leg_x, -y1 + extend_leg_y + adjust_y + step_length * ratio_right, rb.adjust + step_height,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_left, lb.adjust,
                      x0, -y1 - adjust_y + step_length / 2 * ratio_left, lf.adjust,
                      all_height,
    ...
    Output_Coordinate(x0, -y1 - adjust_y, rf.adjust,
                      x0 + extend_leg_x, -y1 - extend_leg_y + adjust_y, rb.adjust + step_height,
                      x0, -y1 + adjust_y + step_length / 2 * ratio_left, lb.adjust,
                      x0, -y1 - adjust_y + step_length / 2 * ratio_left, lf.adjust,
                      all_height,
