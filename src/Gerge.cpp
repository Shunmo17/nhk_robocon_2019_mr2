...
void Receive_gerge()
{
    ...
    */
    /*******************距離センサ********************/
    double gerge_sensor_initial(0.0);
    double gerge_sensor_now(0.0);
    for (int i(0); i < 10; i++)
    {
        gerge_sensor_initial += gerge_sensor.read();
        wait(0.1);
    }
    gerge_sensor_initial /= 10;
    pc.printf("gerge_sensor_initial = %2.2lf\n", gerge_sensor_initial);
    oled.clear();
    oled.printf("Waiting Gerge");
    do
    { //最初のセンサ値との差が以上になったらゲルゲを受け取ったと認識
        gerge_sensor_now = 0.0;
        for (int i(0); i < 10; i++)
        {
            gerge_sensor_now += gerge_sensor.read();
            wait(0.05);
        }
        gerge_sensor_now /= 10;
        pc.printf("gerge_sensor = %2.2lf\n", gerge_sensor_now);
    } while (abs(gerge_sensor - gerge_sensor_initial) < 0.2);
    /*************************************************/
    //Hold_gerge();
    oled.clear();
